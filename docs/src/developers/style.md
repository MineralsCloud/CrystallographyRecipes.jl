...
!!! info
    The style guide is always a work in progress, and not all CrystallographyRecipes code
    follows the rules. When modifying CrystallographyRecipes, please fix the style violations
...
`CrystallographyRecipes` uses [`JuliaFormatter`](https://github.com/domluna/JuliaFormatter.jl) as
an auto-formatting tool.

We use the options contained in [`.JuliaFormatter.toml`](https://github.com/MineralsCloud/CrystallographyRecipes.jl/blob/main/.JuliaFormatter.toml).

To format your code, `cd` to the `CrystallographyRecipes` directory, then run:
...
!!! info
    A continuous integration check verifies that all PRs made to `CrystallographyRecipes` have
