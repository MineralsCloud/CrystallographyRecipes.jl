...
[`CrystallographyRecipes`](https://github.com/MineralsCloud/CrystallographyRecipes.jl).
...
## Install `CrystallographyRecipes`
...
2. Run the following commands and wait for them to finish:
   ...
   julia> Pkg.add("CrystallographyRecipes")
   ```

3. Run

   ```julia-repl
   julia> using CrystallographyRecipes
   ```
...
version of `CrystallographyRecipes`, type

```@repl
using Pkg
Pkg.update()
pkg"add https://github.com/MineralsCloud/CrystallographyRecipes.jl"
```

in the second step above.

## Update `CrystallographyRecipes`
...
our [GitHub repository](https://github.com/MineralsCloud/CrystallographyRecipes.jl)
for new releases.
Once we release a new version, you can update `CrystallographyRecipes` by typing

```@repl
using Pkg
Pkg.update("CrystallographyRecipes")
Pkg.gc()
```

in the Julia REPL.

## Uninstall and reinstall `CrystallographyRecipes`
...
1. To uninstall, in a Julia session, run
   ...
   julia> Pkg.rm("CrystallographyRecipes")

   julia> Pkg.gc()
   ```

2. Press `ctrl+d` to quit the current session. Start a new Julia session and
   reinstall `CrystallographyRecipes`.
